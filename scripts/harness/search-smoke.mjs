...
for (const recipeSlug of ["format-api-response", "extract-json-field", "decode-api-token", "debug-redirect", "check-search-discovery-readiness", "inspect-image-data-url", "check-dns-deployment", "create-wifi-qr", "generate-secure-token", "verify-webhook-signature", "review-security-headers", "validate-deploy-config", "clean-csv-for-api"]) {
  if (!workflows.includes(`slug: "${recipeSlug}"`)) failures.push(`workflow recipe registry missing ${recipeSlug}`);
}
for (const intent of ["api response formatter", "api response report", "json response report", "api error report", "json error line column", "json parse error context", "json field extractor", "json path extractor", "recursive jsonpath", "jwt exp checker", "jwt authorization header", "jwt sensitive claims", "jwt redacted payload", "jwt issuer checker", "jwt audience checker", "jwt scope checker", "jwt auth report", "api token handoff report", "token auth report", "base64 payload report", "base64 decode report", "base64 json report", "hash signature report", "hmac handoff report", "hmac verification report", "redirect chain checker", "site indexing checklist", "google indexing checker", "search console sitemap", "search discovery report", "sitemap indexing check", "sitemap submission report", "robots crawl report", "robots txt report", "crawl policy report", "meta crawler report", "seo meta report", "social preview report", "open graph crawler report", "link preview qa report", "url canonical report", "clean url report", "utm cleanup report", "robots canonical check", "search discovery checklist", "base64 image decoder", "download base64 image", "data url decoder", "dns propagation checklist", "dns deployment checker", "dns deployment checklist", "dns deployment report", "domain dns report", "dmarc checker", "name server checker", "wifi qr code generator", "qr quiet zone", "csrf token generator", "security headers checker", "security response header checker", "hsts checker", "referrer policy checker", "content security policy generator", "docker compose validator", "deployment config checker", "csv cleaner", "markdown table generator"]) {
  if (!workflows.includes(intent)) failures.push(`workflow recipe search intent missing ${intent}`);
}
for (const intent of ["search discovery report", "sitemap submission report", "search console sitemap report"]) {
...
for (const intent of ["meta crawler report", "seo meta report", "social preview report", "open graph crawler report", "link preview qa report"]) {
  if (!registry.includes(intent)) failures.push(`Meta Tag Generator registry search surface missing ${intent}`);
}
for (const intent of ["url canonical report", "clean url report", "utm cleanup report"]) {
