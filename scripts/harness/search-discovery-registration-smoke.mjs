...
for (const fragment of [
  ...
  "Same-day `sc-domain:bobob.app` recheck first showed the older `84`-page row from `2026. 7. 6.`",
  ...
  "Latest `bobob935@gmail.com` check showed clicks `1`, impressions `28`, CTR `3.6%`, average position `3.5` for `3개월`",
  "Google Search Console page indexing",
  "Latest `bobob935@gmail.com` recheck still showed indexed pages `1`, not-indexed pages `32`, last updated `2026. 6. 30`",
  "`크롤링됨 - 현재 색인이 생성되지 않음`: `24`",
  "A `2026-07-09` drilldown check confirmed `크롤링됨 - 현재 색인이 생성되지 않음` is the active visible reason for `24` examples",
  "while the visible `/blog/boring-maintenance-is-content-too` example is now `noindex` and absent from `/sitemaps/en`",
  "Google URL Inspection",
  "Homepage `https://www.bobob.app/` is `URL이 Google에 등록되어 있음` and `페이지 색인이 생성됨`",
  "`/blog`, `/play`, and `/tools/http-status-checker` are not registered",
  "each showed `페이지 색인이 생성되지 않음: 크롤링됨 - 현재 색인이 생성되지 않음`",
  "`/tools/http-status-checker` also received a fresh `색인 생성 요청됨` confirmation",
  "Google states repeated submissions do not improve queue position or priority",
  "Public HTTP crawler-profile evidence",
  "Deployed `946fe35b` exposes fixed request profiles `public`, `googlebot-smartphone`, and `google-inspection-mobile`",
  "without arbitrary user-agent input",
  "Bing and IndexNow",
  `Latest IndexNow representative submission: \`${latestIndexNowSubmissionCount}\` URLs with response \`200\``,
  "Bing Webmaster Tools",
  "Latest `2026-07-08` browser recheck still reached the public Bing Webmaster Tools about/landing page with `Sign In`",
  "Public Bing search",
  "Latest browser check hit a `계속하려면 아래 과제 해결` challenge",
  "Naver Search Advisor",
  "Latest valid `2026-07-07` signed-in Search Advisor recheck reached the `풀꽃` account dashboard",
  "The sitemap-submit page still shows only old broad locale rows from `26.06.16`",
  "`사이트맵을 찾을 수 없습니다`",
  "`sitemaps/en` is not visible in the submitted sitemap row list",
  ...
  "`2026-07-02`",
  "`2026-07-09`",
  "`2026-07-09` drilldown note: `크롤링됨 - 현재 색인이 생성되지 않음` means Google crawled the page but has not indexed it",
  "`boring-maintenance-is-content-too` is a deliberate noindex/archive example and is not in `/sitemaps/en`",
...
for (const fragment of [
  ...
  "the visible Page indexing drilldown is `크롤링됨 - 현재 색인이 생성되지 않음`, not a crawl-failure row",
  "Live Googlebot check: `https://www.bobob.app/blog`, `https://www.bobob.app/play/office-survival`, `https://www.bobob.app/play`, and `https://www.bobob.app/tools` currently return final `200`",
  "`https://www.bobob.app/blog/boring-maintenance-is-content-too` currently returns final `200` with canonical self-reference but `robots=noindex, follow`",
  "added exact Korean Search Console reason queries such as `크롤링됨 - 현재 색인이 생성되지 않음`",
  ...
  "a production `/api/http-status` request with `requestProfile=google-inspection-mobile` returned final `200`",
