...
- Pointer-reactive visual effects must stay lightweight CSS/React, respect reduced motion, and avoid adding WebGL, `ogl`, or `framer-motion` unless explicitly approved. Home/tool directory and tool detail pages should keep the restrained grid-line pointer background, not decorative blobs. Smooth mouse following should be done with requestAnimationFrame-driven CSS variables, not a new animation dependency.
