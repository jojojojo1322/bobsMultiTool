...
- Feature additions should stay inside high-demand clusters. `json-escape-unescape` is the first approved JSON-cluster breadth addition; remaining candidates should be validators, parsers, generators, or cleaners that strengthen existing acquisition paths rather than standalone thin pages.
