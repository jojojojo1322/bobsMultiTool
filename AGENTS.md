...
- Feature additions should stay inside high-demand clusters. `json-escape-unescape`, `yaml-validator`, and `env-parser-validator` are approved data/config-cluster breadth additions; remaining candidates should be validators, parsers, generators, or cleaners that strengthen existing acquisition paths rather than standalone thin pages.
