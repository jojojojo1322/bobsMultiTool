  ...
  return (
    <div className="mt-6 rounded-md border bg-muted/20 p-3" data-play-result-links>
      ...
      <div className={`mt-3 grid gap-2 ${compact ? "" : "sm:grid-cols-2"}`}>
        {relatedPlayLinks.slice(0, compact ? 1 : 2).map((play) => (
          <Link key={play.slug} href={`/play/${play.slug}`} className="rounded-md border bg-background p-3 text-sm hover:bg-muted" data-play-related-play>
            ...
            {play.description && !compact ? <span className="mt-1 block text-xs leading-5 text-muted-foreground">{play.description}</span> : null}
          </Link>
        ))}
        {relatedBlogLinks.slice(0, compact ? 1 : 2).map((post) => (
          <Link key={post.slug} href={`/blog/${post.slug}`} className="rounded-md border bg-background p-3 text-sm hover:bg-muted" data-play-related-blog>
            ...
            {post.description && !compact ? <span className="mt-1 block text-xs leading-5 text-muted-foreground">{post.description}</span> : null}
          </Link>
        ))}
