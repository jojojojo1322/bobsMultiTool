...
export function moleWhackOutcome(target: MoleTarget): MoleWhackOutcome {
  ...
  if (target.good) {
    if (timing.remaining >= 0.48) return { score: 4, focus: 4, detail: "볼 알림 빠르게 확인" };
    if (timing.remaining <= 0.2) return { score: 2, focus: 1, detail: "늦게라도 볼 알림 확인" };
    return { score: 3, focus: 3, detail: "지금 볼 알림만 확인" };
  }
  if (timing.urgency > 0.72) return { score: -5, focus: -13, detail: "꺼둘 알림에 손이 감" };
  return { score: -4, focus: -11, detail: "꺼둘 알림을 굳이 누름" };
}
