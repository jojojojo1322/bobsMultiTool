...
function snapshot(state: GameState): ViewState {
  return {
    ...
    memoryShowing: state.memoryShowing,
...
function drawFlight(content: ArcadeGameContent, state: GameState, ctx: CanvasRenderingContext2D) {
  ...
  backdrop.addColorStop(0.56, "rgba(44,42,34,0.96)");
  backdrop.addColorStop(1, "rgba(17,16,13,1)");
  ctx.fillStyle = backdrop;
  ctx.fillRect(0, 0, canvasWidth, canvasHeight);

  ctx.strokeStyle = "rgba(216,173,85,0.12)";
  ctx.lineWidth = 1;
  ...
  ctx.fillStyle = "rgba(38,35,28,0.72)";
  ctx.beginPath();
  ctx.roundRect(26, 20, canvasWidth - 52, 44, 14);
  ctx.fill();
  ctx.strokeStyle = "rgba(216,173,85,0.22)";
  ctx.stroke();
  ...
  ctx.fill();
  ctx.fillStyle = "rgba(216,173,85,0.18)";
  ctx.beginPath();
  ...
  if (incomingSprite) {
    ...
    ctx.fillStyle = incomingSprite.good ? "rgba(120,167,161,0.24)" : "rgba(184,91,74,0.24)";
    ctx.beginPath();
    ctx.roundRect(Math.min(canvasWidth - 176, Math.max(state.playerX + 42, incomingSprite.x - 74)), incomingSprite.y - 28, 142, 24, 12);
    ctx.fill();
    ctx.strokeStyle = incomingSprite.good ? "rgba(120,167,161,0.55)" : "rgba(184,91,74,0.58)";
    ctx.lineWidth = 1;
    ctx.stroke();
    ctx.fillStyle = "#f4ead2";
  ...
  ctx.fillStyle = "rgba(38,35,28,0.58)";
  ctx.beginPath();
  ctx.roundRect(48, altitudeTop - 10, 18, altitudeRange + 20, 9);
  ctx.fill();
  ctx.strokeStyle = "rgba(216,173,85,0.2)";
  ctx.lineWidth = 1;
  ctx.stroke();
  ctx.fillStyle = "rgba(244,234,210,0.28)";
  ctx.beginPath();
  ...
  for (const sprite of state.sprites) {
    ...
    ctx.fill();
    if (sprite.good) {
      const ductWidth = sprite.radius * 2 + 38;
      const slabHeight = 18;
      const duct = ctx.createLinearGradient(-ductWidth / 2, -sprite.radius - 30, ductWidth / 2, sprite.radius + 30);
      duct.addColorStop(0, "rgba(152,145,124,0.94)");
      duct.addColorStop(0.5, "rgba(48,46,38,0.96)");
      duct.addColorStop(1, "rgba(120,167,161,0.82)");
      ctx.fillStyle = duct;
      ctx.beginPath();
      ctx.roundRect(-ductWidth / 2, -sprite.radius - 28, ductWidth, slabHeight, 4);
      ctx.roundRect(-ductWidth / 2, sprite.radius + 10, ductWidth, slabHeight, 4);
      ctx.fill();
      ctx.strokeStyle = "rgba(120,167,161,0.72)";
      ctx.lineWidth = 2;
      ctx.stroke();
      ctx.fillStyle = "rgba(120,167,161,0.18)";
      ctx.beginPath();
      ctx.roundRect(-sprite.radius - 8, -sprite.radius + 2, sprite.radius * 2 + 16, sprite.radius * 2 + 6, 12);
      ctx.fill();
    } else {
      ...
      cloud.addColorStop(1, "rgba(88,34,26,0.95)");
      ctx.fillStyle = cloud;
      ctx.beginPath();
    ...
    ctx.fillStyle = sprite.good ? "#f4ead2" : "#fff1dc";
  ...
  ctx.fillStyle = liftActive ? "rgba(216,173,85,0.28)" : "rgba(120,167,161,0.18)";
  ctx.beginPath();
  ctx.ellipse(-32, 10, 42, 14, -0.16, 0, Math.PI * 2);
  ctx.fill();
  const body = ctx.createLinearGradient(-28, -20, 28, 18);
  body.addColorStop(0, "#f4ead2");
  body.addColorStop(0.48, primary);
  body.addColorStop(1, "rgba(120,167,161,0.95)");
  ctx.fillStyle = body;
  ctx.beginPath();
  ctx.moveTo(34, 0);
  ctx.lineTo(-24, -23);
...
function drawShooter(content: ArcadeGameContent, state: GameState, ctx: CanvasRenderingContext2D) {
  ...
  if (signalMode) {
    ...
    ctx.lineWidth = 1;
    const cityBaseY = dangerLineY + 40;
    for (let x = 56; x < canvasWidth - 88; x += 86) {
      ctx.beginPath();
      ctx.roundRect(x, cityBaseY - 14, 18, 14, 2);
      ctx.roundRect(x + 22, cityBaseY - 22, 16, 22, 2);
      ctx.roundRect(x + 42, cityBaseY - 10, 22, 10, 2);
      ctx.stroke();
    }
  ...
  if (mode === "missile") {
    const impactReads = state.sprites
      .filter((sprite) => sprite.vy > 0 && sprite.y < dangerLineY)
      .map((sprite) => {
        const secondsToLine = (dangerLineY - sprite.y) / sprite.vy;
        return {
          sprite,
          secondsToLine,
          impactX: clamp(sprite.x + sprite.vx * secondsToLine, 44, canvasWidth - 44),
        };
      })
      .sort((left, right) => left.secondsToLine - right.secondsToLine);
    const firstRealThreat = impactReads.find((item) => item.sprite.good);

    for (const item of impactReads.slice(0, 5)) {
      const { sprite, impactX } = item;
      const first = firstRealThreat?.sprite.id === sprite.id;
      ctx.save();
      ctx.strokeStyle = sprite.good ? (first ? "rgba(248,113,113,0.58)" : "rgba(248,113,113,0.3)") : "rgba(148,163,184,0.2)";
      ctx.lineWidth = first ? 2 : 1;
      ctx.setLineDash(sprite.good ? [7, 6] : [3, 8]);
      ctx.beginPath();
      ctx.moveTo(sprite.x, sprite.y + sprite.radius);
      ctx.lineTo(impactX, dangerLineY + 2);
      ctx.stroke();
      ctx.setLineDash([]);
      ctx.fillStyle = sprite.good ? (first ? "rgba(248,113,113,0.9)" : "rgba(248,113,113,0.5)") : "rgba(148,163,184,0.42)";
      ctx.beginPath();
      ctx.roundRect(impactX - 7, dangerLineY + 5, 14, 18, 4);
      ctx.fill();
      if (first) {
        ctx.fillStyle = "rgba(254,226,226,0.96)";
        ctx.beginPath();
        ctx.roundRect(clamp(impactX - 36, 46, canvasWidth - 118), dangerLineY - 22, 72, 18, 9);
        ctx.fill();
...
function drawMemory(content: ArcadeGameContent, state: GameState, ctx: CanvasRenderingContext2D) {
  ...
  const statusTitle = state.memoryShowing ? "보기 중" : "누르기 대기";
  const statusDetail = state.memoryShowing ? "손 떼고 켜진 램프 순서 보기" : "같은 순서로 램프 누르기";
  ...
  ctx.fillText(`보기 ${state.memoryRound} · 램프 ${sequenceLength}`, canvasWidth - 42, 43);
  ...
  ctx.font = "850 12px system-ui, -apple-system, BlinkMacSystemFont, sans-serif";
  ctx.textAlign = "left";
  ctx.fillText(state.memoryShowing ? "본 순서" : "누른 순서", railX + 16, railY + 24);
  ...
  for (let index = 0; index < sequenceLength; index += 1) {
    ...
    if (index > 0 && index % 3 === 0) {
      const dividerX = slotX - slotGap / 2;
      ctx.strokeStyle = "rgba(255,255,255,0.22)";
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(dividerX, railY + 35);
      ctx.lineTo(dividerX, railY + 58);
      ctx.stroke();
      ctx.fillStyle = "rgba(255,255,255,0.44)";
      ctx.font = "700 9px system-ui, -apple-system, BlinkMacSystemFont, sans-serif";
      ctx.textAlign = "center";
      ctx.fillText("3칸", dividerX, railY + 68);
    }
  ...
  ctx.font = "850 12px system-ui, -apple-system, BlinkMacSystemFont, sans-serif";
  ctx.textAlign = "right";
  ctx.fillText(state.memoryShowing ? `${playbackStep}/${sequenceLength} 보기` : `${state.memoryInput.length}/${sequenceLength} 누름`, railX + railWidth - 16, railY + 24);
  ...
  ctx.textAlign = "left";
  ctx.fillText(state.memoryShowing ? "켜진 램프 보기" : hasMemoryInput ? "누른 순서 이어가기" : "첫 램프 누르기", panelX + 18, 118);
  ctx.fillStyle = "rgba(255,255,255,0.62)";
  ctx.font = "700 12px system-ui, -apple-system, BlinkMacSystemFont, sans-serif";
  ctx.fillText(state.memoryShowing ? "켜진 램프를 3칸씩 끊어 봅니다." : `누를 자리 ${nextSlot}부터 이어 누릅니다.`, panelX + 18, 146);

  ctx.fillStyle = "rgba(255,255,255,0.08)";
  ctx.beginPath();
  ctx.roundRect(panelX + 18, 158, 176, 22, 11);
  ctx.fill();
  ctx.fillStyle = state.memoryShowing ? "rgba(250,204,21,0.9)" : "rgba(255,255,255,0.52)";
  ctx.font = "850 10px system-ui, -apple-system, BlinkMacSystemFont, sans-serif";
  ctx.textAlign = "center";
  ctx.fillText("1 보기", panelX + 56, 173);
  ctx.fillStyle = !state.memoryShowing ? "rgba(190,242,100,0.9)" : "rgba(255,255,255,0.52)";
  ctx.fillText("2 같은 순서 누르기", panelX + 138, 173);
  ...
  ctx.fillStyle = "rgba(255,255,255,0.62)";
  ctx.font = "700 12px system-ui, -apple-system, BlinkMacSystemFont, sans-serif";
  ctx.fillText(state.memoryShowing ? "보기 끝나면 누르기 대기가 켜집니다." : hasMemoryInput ? "맞은 램프가 누른 순서 줄에 남았습니다." : "첫 램프를 고를 차례입니다.", panelX + 18, 224);
  ctx.fillText("맞은 램프는 선과 순서 줄로 남습니다.", panelX + 18, 252);
  ...
  ctx.font = "650 12px system-ui, -apple-system, BlinkMacSystemFont, sans-serif";
  ctx.textAlign = "left";
  ctx.fillText("보기 중에는 손을 멈추고, 누르기 대기에서 마우스/방향키/숫자 1~9로 같은 램프를 누릅니다. 헷갈리면 R.", 34, canvasHeight - 20);
  ...
  if (!state.started) {
    ...
    ctx.fillText(content.title, canvasWidth / 2, 164);
    ctx.font = "500 15px system-ui, -apple-system, BlinkMacSystemFont, sans-serif";
    ctx.fillText("보기 중에는 누르지 말고 램프 순서만 봅니다.", canvasWidth / 2, 202);
    ctx.fillText("누르기 대기가 켜지면 같은 순서로 누릅니다. 헷갈리면 R로 다시 봅니다.", canvasWidth / 2, 228);
  }
}
