...
const arcadeContextGuides = {
  ...
  flight: {
    title: "파란 덕트 틈과 붉은 케이블 벽을 먼저 보는 원버튼 통과판",
    ...
    pacingText: "한 판은 1분 서버실 덕트 통과로 갑니다. 결과는 점수보다 누르고 떼는 길이와 덕트 틈 통과로 남습니다.",
    recordText: "기록은 다음 덕트 틈 높이선을 더 빨리 읽기 위한 흔적입니다. 팬 바람을 과하게 타지 않고 붉은 케이블 벽을 피하는 감각만 남기면 됩니다.",
  },
  "brick-breaker": {
  ...
  snake: {
    title: "머리 앞 한 칸을 보고 꼬리 출구를 남기는 배포줄",
    firstLook: "배포줄 머리, 자기 꼬리, 노란 배포 조각, 머리 앞 한 칸 예고를 먼저 봅니다.",
    firstMove: "첫 움직임은 방향 전환입니다. 노란 조각만 따라가지 말고 바로 앞 칸이 벽인지 꼬리인지 보고 꺾습니다.",
  ...
  mole: {
    title: "긴급 카드와 소음 카드를 나누는 알림 스위치보드",
    firstLook: "긴급 카드, 소음 카드, NOW/MUTE 배지, 남은 시간 링, 선별 커서를 먼저 봅니다.",
    firstMove: "첫 움직임은 확인입니다. 소음 카드는 일부러 꺼둔 채 보내고 지금 볼 긴급 카드만 누릅니다.",
    pacingText: "한 판은 1분 알림 선별입니다. 결과는 많이 누른 수보다 지금 볼 카드만 확인했는지로 남습니다.",
    recordText: "기록은 다음 알림 기준을 줄이기 위한 흔적입니다. 전부 누르려 했다면 손을 늦추고 MUTE 카드를 그냥 보내는 쪽으로 다시 봅니다.",
  },
  stacker: {
  ...
  growth: {
    title: "제작대, 납품 상자, 병목표, 설비 장부가 이어지는 성장판",
    firstLook: "왼쪽 제작대, 아래 납품 상자 레일, 오른쪽 설비 장부, 상단 병목표와 열 게이지를 먼저 봅니다.",
    firstMove: "첫 움직임은 제작대입니다. 부품을 만들어 상자를 채우고, 병목표가 과열이면 냉각팬, 부품 부족이면 제작대를 먼저 봅니다.",
    pacingText: "이 판은 자동으로 결과 화면으로 넘어가지 않습니다. 출력·냉각·자동·장비 설비 줄과 열 병목을 보며 같은 작업장을 계속 키웁니다.",
    recordText: "기록은 납품 흐름, 상자 진행도, 병목표, 설비 선택만 남깁니다. 조작은 제작대, 설비 장부, 작업 배분판처럼 화면 안 표면에서 끝납니다.",
  },
...
function playContextGuide(content: PlayContent): PlayContextGuide {
  if (content.type === "arcade-game") {
    const arcadeContextGuideOverrides: Partial<Record<string, Omit<PlayContextGuide, "kicker">>> = {
      "ten-box-rush": {
        title: "숫자 사과를 훑어 합 10길을 비우는 사과상자",
        firstLook: "사과상자 격자, 숫자 사과, 현재 합계, 노란 사과 10길 힌트를 먼저 봅니다.",
        firstMove: "첫 움직임은 드래그입니다. 1+9 짝만 집지 말고 2+3+5처럼 세 개 이상 이어지는 사과길도 훑습니다.",
        pacingText: "한 판은 1분 사과상자 비우기입니다. 결과는 숫자 보상보다 어떤 사과길을 보고 빈칸을 열었는지로 남습니다.",
        recordText: "기록은 산수 점검표가 아니라 다음 사과길을 더 빨리 찾기 위한 손길 메모입니다. 넘쳤다면 손을 되돌릴 지점을 다시 봅니다.",
      },
      "bug-clicker": {
      ...
      "deploy-missile-defense": {
        title: "아래 릴리스 도시선에 먼저 닿을 충돌점부터 막는 요격판",
        firstLook: "위에서 내려오는 경고 궤적, 아래 릴리스 도시선, 조준점, 충돌점까지 남은 거리를 먼저 봅니다.",
        firstMove: "첫 움직임은 하단 요격포 조준입니다. 화면 위의 먼 궤적보다 아래 도시선에 먼저 닿을 충돌점에 선을 맞춥니다.",
  ...
  if (content.type === "tap-game") {
    if (content.slug === "ai-review-tap") {
      return {
        kicker: "첫 5초 안내",
        title: "AI 답변 전표에 확인 더 필요/근거 확인됨 도장을 찍는 검수판",
        firstLook: "AI 답변 전표, 경로 열림·명령 결과·원문 출처·한계 표시 체크줄, 확인 더 필요 도장, 근거 확인됨 도장을 먼저 봅니다.",
        firstMove: "첫 움직임은 도장입니다. 경로 열림·명령 결과·원문 출처·한계 표시가 비면 확인 더 필요, 실제 확인 흔적이 있으면 근거 확인됨을 찍습니다.",
        pacingText: `${content.durationLabel} 동안 빠르게 맞히기보다 확인 흔적이 있는 말과 매끈한 단정을 나눕니다.`,
        recordText: "기록은 숫자 결과보다 검수 전표에 가깝습니다. 도장을 잘못 찍었다면 다음 문장에서 경로, 명령, 원문, 한계 중 무엇을 먼저 열어볼지 다시 봅니다.",
      };
    }
  ...
  if (content.slug === "office-survival") {
    return {
      ...
      firstLook: "근무 시각, 마감 위험, 몸 배터리·마음 여유·일더미·신뢰 잔고 막대, 줄일 것/지킬 것/빌릴 것 메모를 먼저 봅니다.",
      firstMove: "첫 움직임은 좋아 보이는 답을 고르는 것이 아니라, 지금 전표에서 요구를 낮출지 통제를 되찾을지 지원을 요청할지 판단하는 것입니다.",
