...
const arcadeContextGuides = {
  ...
  lottery: {
    title: "은박 복권 한 장을 긁고 표식 전표를 보는 판",
    firstLook: "현재 복권지 이름, 은박 9칸, 오른쪽 표식표, 아래 한 장 기록 전표가 어디에 남는지 먼저 봅니다.",
    firstMove: "첫 움직임은 긁기입니다. 한 장을 다 보면 바로 다음 장보다 같은 그림 줄, 즉석 표식, 한 장 기록 전표를 먼저 읽습니다.",
    pacingText: "이 판은 끝나는 순위판이 아니라 한 장씩 이어지는 은박 복권지입니다. 더 긁고 싶으면 다음 장으로 가고, 멈추고 싶으면 그 자리에서 멈춥니다.",
    recordText: "현재 공개 버전은 실제 돈, 결제, 현금화가 없습니다. 기록은 한 장의 표식 전표와 긁힘 로그만 남기고 손실 만회 문구로 밀어붙이지 않습니다.",
  },
  "lottery-economy": {
    title: "금화 복권 빚장부: 지갑, 빚, 확률표를 먼저 보는 판",
    firstLook: "위쪽 지갑·빚·손익 도장, 10/25/55금화 전표, 오른쪽 확률표를 먼저 봅니다.",
    firstMove: "첫 움직임은 10금화 전표 지정, 한 장 사기, 은박 긁기입니다. 지급 전표가 열리면 바로 다음 장보다 지갑과 빚을 먼저 봅니다.",
    pacingText: "복권은 한 장씩 갑니다. 이전 지급이 다음 장을 보장하지 않고, 손실이 커지면 10금화로 낮아지며 부족하면 재기자금은 한 번만 열립니다.",
    recordText: "실제 돈, 결제, 현금화는 없습니다. 기록은 당첨 자랑보다 비용, 지급, 평균손익, 빚, 멈춤 판단을 남깁니다.",
  },
  "match-three": {
  ...
  mole: {
    ...
    firstLook: "볼 알림, 꺼둘 알림, 보기/꺼둠 배지, 닫히는 시간 링, 선별 커서를 먼저 봅니다.",
    firstMove: "첫 움직임은 찍기입니다. 꺼둘 알림은 일부러 보내고 지금 볼 알림만 찍습니다.",
    pacingText: "한 판은 1분 알림 선별입니다. 복기는 많이 누른 수보다 지금 볼 알림만 집었는지로 남습니다.",
...
function playContextGuide(content: PlayContent): PlayContextGuide {
  if (content.type === "arcade-game") {
    const arcadeContextGuideOverrides: Partial<Record<string, Omit<PlayContextGuide, "kicker">>> = {
      ...
      "deploy-invaders": {
        ...
        pacingText: "한 판은 1분 릴리스 게이트 방어입니다. 복기는 발사량보다 게이트선 가까운 표식 행렬을 골라 차단한 흐름으로 남습니다.",
        recordText: "기록은 화력표가 아니라 게이트선 메모입니다. 미끼에 흔들렸다면 다음 판에서는 방어포를 더 늦게 움직입니다.",
      },
      "deploy-missile-defense": {
        title: "아래 릴리스 도시선으로 떨어지는 첫 충돌핀을 요격하는 방어포",
        firstLook: "위에서 내려오는 경고 궤적, 아래 릴리스 도시선, 조준점, 첫 충돌핀을 먼저 봅니다.",
        firstMove: "첫 움직임은 하단 방어포 조준입니다. 위쪽 먼 궤적보다 아래 도시선에 먼저 닿을 충돌핀에 선을 맞춥니다.",
        pacingText: "한 판은 1분 릴리스 도시선 방어입니다. 복기는 발사량보다 도시선에 닿을 경고 궤적을 먼저 지운 흐름으로 남습니다.",
  ...
  if (content.slug === "office-survival") {
    return {
      ...
      pacingText: `${content.durationLabel} 동안 하루 전표를 처리합니다. 마감 기록은 빠른 탈출보다 어떤 자원을 방치했고 어떤 부탁을 줄였는지로 남습니다.`,
      recordText: "퇴근 기록지는 자랑판이 아니라 다음 하루를 덜 태우기 위한 장부입니다. 일더미만 낮추려다 몸 배터리, 마음 여유, 신뢰 잔고 중 무엇을 태웠는지 다음 판에서 다시 봅니다.",
    };
  }
