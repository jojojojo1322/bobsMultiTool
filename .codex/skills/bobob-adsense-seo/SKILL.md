...
- Use `npm run seo:indexing-followup` before the next Search Console/Bing pass. It writes a gitignored follow-up packet with the live sitemap/feed/OpenSearch/robots snapshot, the `bobob935@gmail.com` Search Console property link, representative URL inspection targets, Bing recommendation checks, and the stop rule that discovery submissions are not indexing proof.
