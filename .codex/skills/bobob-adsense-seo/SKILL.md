...
- Use `npm run seo:indexing-followup` before the next Search Console/Bing pass. It writes a gitignored follow-up packet with the live sitemap/feed/OpenSearch/robots snapshot, the `bobob935@gmail.com` Search Console property link, the Chrome profile/session guard for that account, representative URL inspection targets, Bing recommendation checks, and the stop rule that discovery submissions are not indexing proof.
- Keep `docs/search-discovery-registration.md` current after sitemap, feed, IndexNow, WebSub, OpenSearch, llms, robots, Search Console, or Bing registration work. Run `npm run harness:search-discovery-registration` to prove the matrix matches the live discovery surface and the latest submitted counts.
