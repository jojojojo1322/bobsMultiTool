...
- HTTP Status Checker UI should render a status summary, ordered redirect chain, hop timing/cache details, redirect diagnostics, allowlisted final response headers, a copyable public URL report, a local HTTP header parser for pasted header blocks, security header readiness, a copyable security header report, a CSP generator with warnings and parser handoff, and raw JSON response.
