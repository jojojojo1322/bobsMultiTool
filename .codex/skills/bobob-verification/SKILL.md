...
- Pointer-reactive background motion should respect reduced motion, use requestAnimationFrame-driven CSS variable smoothing with depth offset and ray opacity variables, and keep the approved ReactBits route mapping: Light Rays on main home routes, Galaxy on tool directory, tool detail, guide, trust, legal, and other non-home routes. Do not add `framer-motion` or another animation dependency without explicit approval.
- Verify ReactBits tuning against the approved reference settings: Light Rays top origin, five long broad rays, speed 3, spread 3.35, ray length 4.6, fade distance 2, saturation 1.1, mouse influence 1, and no noise/distortion; Galaxy mouse interaction and repulsion, density 3, glow 0.3, saturation 0, hue shift 100, twinkle 0.1, rotation speed 0.05, repulsion strength 0.5, star speed 0.1, and animation speed 0.5. Check both light and dark modes so the effect does not disappear or overpower the workbench in either theme.
- On mobile/detail routes, verify the ReactBits canvas CSS size and backing pixel size are viewport-bounded, not document-height sized.
- Run `npm run harness:pointer-background` after pointer background changes to prove CSS variables change on real mouse movement and the Light Rays/Galaxy route mapping is intact.
