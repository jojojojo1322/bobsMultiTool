...
- Feature additions should stay inside high-demand clusters. `json-escape-unescape` is the first approved JSON-cluster breadth addition; next candidates should be YAML Validator, ENV Parser/Validator, Docker Compose Validator/Formatter, Regex Generator from examples, HTTP Header Parser, CSP Generator, Redirect Chain Checker improvements, Markdown Table Generator, and CSV Cleaner.
