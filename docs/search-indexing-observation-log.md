...
- Account switch retry: adding `authuser=bobob935@gmail.com` still resolved to `task10@ljfriends.net`; Google AccountChooser with `bobob935@gmail.com` then reached the Google sign-in identifier screen instead of an already-authenticated Search Console report.
