...
| --- | --- | --- | --- | --- |
| Google Search Console sitemap | `/sitemaps/en` under the `https://www.bobob.app/` property | Current reduced live sitemap target: `77` URLs after promoting owner/Play-context representative posts, adding dedicated memory-console, meeting-routine, priority-board, AI-review, prompt-request, and bug-ticket build logs, and moving eight duplicate date-sensitive information posts out of the representative set. Latest signed-in Search Console observation after resubmission: `/sitemaps/en`, submitted `2026. 7. 3.`, last read `2026. 7. 3.`, status `성공`, discovered pages `77`, videos `0` | Google accepted, read, and discovered the current 77-URL canonical sitemap set | It does not prove indexing, impressions, ranking, or that the representative Blog + Play pages are search-ready |
...
| Bing and IndexNow | `https://api.indexnow.org/indexnow` with the public key file | Latest IndexNow representative submission: `77` URLs with response `200` after the 77-URL deployment refresh | Bing-compatible discovery was refreshed for the current representative URL set | It does not prove Bing indexed or ranked the pages |
...
- Current representative `/sitemaps/en` live URL count: `77`
- Latest IndexNow representative submission count: `77`
- Current representative feed source target item count: `62`
- The current live target now matches the last Search Console discovered count, but this is discovery evidence only.
...
- Indexed pages: `0`
- Not indexed pages: `5`
- `/sitemaps/en` discovered pages: `77` against the current live `77`-URL sitemap; this proves discovery alignment, not indexing.
