...
| --- | --- | --- | --- | --- |
| Google Search Console sitemap | `/sitemaps/en` under the `https://www.bobob.app/` property | Current production sitemap target: `75` URLs after the lucky-scratch duplicate representative consolidation. Latest signed-in Search Console observation after resubmission of the current 75-URL deployment: `/sitemaps/en`, submitted `2026. 7. 3.`, last read `2026. 7. 3.`, status `성공`, discovered pages `75`, videos `0` | Google accepted, read, and discovered the previous 75-URL canonical sitemap set | It does not prove indexing, impressions, ranking, or that the representative Blog + Play pages are search-ready |
...
- Latest external feed publish item count: `60`
- The latest Search Console discovered count now reflects the current `75`-URL sitemap observation. IndexNow and WebSub were refreshed against the same production set with `75` sitemap URLs and `60` feed items, but this is still discovery evidence rather than indexing proof.
...
- Indexed pages: `0`
- Not indexed pages: `5`
- `/sitemaps/en` discovered pages: `75` against the current submitted `75`-URL sitemap; this proves discovery alignment for the current deployment, not indexing.
