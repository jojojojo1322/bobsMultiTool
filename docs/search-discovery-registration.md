...
- Current submitted sitemap URL count: `76`
...
| --- | --- | --- | --- | --- |
| Google Search Console sitemap | `/sitemaps/en` under the `https://www.bobob.app/` property | Current reduced sitemap target: `76` URLs. The `bobob935@gmail.com` session previously resubmitted `sitemaps/en` after the live 77-URL deployment, and Search Console showed `사이트맵이 제출됨`. The visible row now shows `/sitemaps/en`, submitted `2026. 7. 3.`, last read `2026. 7. 3.`, status `성공`, discovered pages `73`, videos `0` | Google accepted and read the canonical sitemap after the trust-page correction | It does not prove indexing, impressions, ranking, or that Search Console has discovered all `76` current sitemap URLs yet |
...
| Bing and IndexNow | `https://api.indexnow.org/indexnow` with the public key file | Current IndexNow representative target: `76` URLs. The latest confirmed IndexNow response remains `200` for the previous reduced representative set | Bing-compatible discovery can be refreshed for the reduced representative URL set | It does not prove Bing indexed or ranked the pages |
...
| RSS feed | `/feed.xml` | Representative feed target has `61` Blog + Play items and WebSub hub discovery | Feed readers and lightweight crawlers can discover the representative content set | It does not replace sitemap or webmaster-tool evidence |
| Atom feed | `/atom.xml` | Representative feed target has `61` Blog + Play entries and WebSub hub discovery | Feed readers and WebSub can discover the representative content set | It does not prove search indexing |
| JSON Feed | `/feed.json` | Representative feed target has `61` Blog + Play items and WebSub hub metadata | Programmatic readers can discover the representative content set | It does not broaden the submitted sitemap scope |
| WebSub | `https://pubsubhubbub.appspot.com/` for RSS and Atom topics | Current WebSub/feed target: `61` RSS items and `61` Atom entries. The latest confirmed WebSub response statuses remain `204`, `204` for the previous reduced feed set | Feed update topics can be refreshed for the representative feed set | It does not prove Google indexed the URLs |
...
- Current representative `/sitemaps/en` URL count: `76`
- Current IndexNow representative target: `76`
- Current representative feed item target: `61`
- Trust-page correction Search Console sitemap submission was accepted and read, but the discovered page count remains `73` until Google reprocesses the current `76`-URL XML.
...
- Indexed pages: `0`
- Not indexed pages: `5`
- `/sitemaps/en` discovered pages: `73` against the current `76`-URL sitemap.
