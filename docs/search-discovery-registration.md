...
- Latest IndexNow representative submission count: `75`
...
| WebSub | `https://pubsubhubbub.appspot.com/` for RSS and Atom topics | Latest WebSub representative publish: `60` RSS items and `60` Atom entries with response statuses `204`, `204` after the 75-URL production deployment | Feed update topics were refreshed for the current representative feed set | It does not prove Google indexed the URLs |
...
- Latest external feed publish item count: `60`
- The latest Search Console discovered count reflects the previously submitted `75`-URL sitemap observation. The current source sitemap target is now `83` URLs after adding selected operations-first tool URLs, so deployment, submitted URL health, Search Console resubmission, IndexNow, and later discovery observation must be refreshed before treating the new target as externally seen.
