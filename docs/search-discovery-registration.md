...
- Current submitted sitemap URL count: `93`
- Current feed item count: `82`
- Current Blog count: `59`
...
| Bing and IndexNow | `https://api.indexnow.org/indexnow` with the public key file | Latest response status `200`; submitted URL count `93` after the Play canvas upgrade, password candidate fix, and follow-up information/content expansion | Bing-compatible discovery ping accepted the current live sitemap URL set | It does not prove Bing indexed or ranked the pages |
...
| RSS feed | `/feed.xml` | Live feed has `82` Blog + Play items and WebSub hub discovery | Feed readers and lightweight crawlers can discover current content | It does not replace sitemap or webmaster-tool evidence |
| Atom feed | `/atom.xml` | Live feed has `82` Blog + Play entries and WebSub hub discovery | Feed readers and WebSub can discover current content | It does not prove search indexing |
| JSON Feed | `/feed.json` | Live feed has `82` Blog + Play items and WebSub hub metadata | Programmatic readers can discover current content | It does not broaden the submitted sitemap scope |
| WebSub | `https://pubsubhubbub.appspot.com/` for RSS and Atom topics | Latest publish responses `204`, `204` with feed item counts `82`, `82` | Feed update topics were accepted by the hub | It does not prove Google indexed the URLs |
...
- Current live `/sitemaps/en` URL count: `93`
- Latest IndexNow URL count: `93`
- Latest feed item count: `82`
