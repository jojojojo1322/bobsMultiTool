...
| --- | --- | --- | --- | --- |
| Google Search Console sitemap | `/sitemaps/en` under the `https://www.bobob.app/` property | Current source sitemap target: `74` URLs after the additional World Cup information prune. Latest signed-in Search Console observation after resubmission of the previous 77-URL deployment: `/sitemaps/en`, submitted `2026. 7. 3.`, last read `2026. 7. 3.`, status `성공`, discovered pages `77`, videos `0` | Google accepted, read, and discovered the latest externally submitted 77-URL canonical sitemap set | It does not prove indexing, impressions, ranking, that the representative Blog + Play pages are search-ready, or that the new 74-URL source target has been deployed, submitted, and observed |
...
- Latest external feed publish item count: `62`
- The latest Search Console discovered count matches the latest externally submitted `77`-URL sitemap, but this is discovery evidence only and does not prove the new 74-URL source target.
...
- Indexed pages: `0`
- Not indexed pages: `5`
- `/sitemaps/en` discovered pages: `77` against the latest externally submitted `77`-URL sitemap; this proves discovery alignment for that deployment, not indexing or the current 74-URL source target.
