...
| Public HTTP crawler-profile evidence | `/tools/http-status-checker` and `/api/http-status` | Deployed `946fe35b` exposes fixed request profiles `public`, `googlebot-smartphone`, and `google-inspection-mobile`; live production checks returned `requestProfile` evidence for the Google InspectionTool mobile profile, and local route smoke covers default, Googlebot Smartphone, and Google InspectionTool request profiles. Deployed `68aa240e` also exposes final HTML indexability signals for canonical match, robots and Googlebot robots policy, title/description length, h1 count, html lang, and noindex; the live HTTP Status Checker self-check returned final `200`, `canonicalMatchesFinal: true`, `h1Count: 1`, `htmlLang: en`, and `Search ready: Ready for indexing request` in the browser. | The public tool can collect reproducible final-status, redirect-chain, allowlisted-header, request-profile, and final HTML indexability evidence without arbitrary user-agent input before Search Console follow-up | It does not prove that Googlebot or URL Inspection actually crawled, queued, indexed, ranked, or showed the checked URL |
| Bing and IndexNow | `https://api.indexnow.org/indexnow` with the public key file | Latest IndexNow representative submission: `85` URLs with response `200` after the HTTP indexability report deployment | Bing-compatible discovery was refreshed for the current deployed representative URL set | It does not prove Bing indexed or ranked the pages, and it does not replace Search Console or Naver observation |
...
- Public HTTP crawler-profile and indexability evidence: `/tools/http-status-checker` now separates default public requests, Googlebot Smartphone, and Google InspectionTool mobile request profiles, and it reports final HTML canonical/robots/title/description/h1/lang/noindex signals before Search Console follow-up. This is crawlability and public URL readiness evidence only, not Search Console indexing proof.
