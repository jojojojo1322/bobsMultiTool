...
| WebSub | `https://pubsubhubbub.appspot.com/` for RSS and Atom topics | Latest WebSub representative publish: `59` RSS items and `59` Atom entries with response statuses `204`, `204` after the meeting-escape archive consolidation refresh | Feed update topics were refreshed for the current representative feed set | It does not prove Google indexed the URLs |
