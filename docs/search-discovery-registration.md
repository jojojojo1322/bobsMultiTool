...
| Public HTTP crawler-profile evidence | `/tools/http-status-checker` and `/api/http-status` | Deployed `946fe35b` exposes fixed request profiles `public`, `googlebot-smartphone`, and `google-inspection-mobile`; live production checks returned `requestProfile` evidence for the Google InspectionTool mobile profile, and local route smoke covers default, Googlebot Smartphone, and Google InspectionTool request profiles. | The public tool can collect reproducible final-status, redirect-chain, allowlisted-header, and request-profile evidence without arbitrary user-agent input | It does not prove that Googlebot or URL Inspection actually crawled, queued, indexed, ranked, or showed the checked URL |
| Bing and IndexNow | `https://api.indexnow.org/indexnow` with the public key file | Latest IndexNow representative submission: `85` URLs with response `200` after the HTTP crawler-profile deployment | Bing-compatible discovery was refreshed for the current deployed representative URL set | It does not prove Bing indexed or ranked the pages, and it does not replace Search Console or Naver observation |
...
- Public HTTP crawler-profile evidence: `/tools/http-status-checker` now separates default public requests, Googlebot Smartphone, and Google InspectionTool mobile request profiles so future URL reachability checks can record which fixed crawler-like header set was used. This is crawlability evidence only, not Search Console indexing proof.
