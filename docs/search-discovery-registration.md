...
- Current submitted sitemap source target URL count: `89`
...
- Current representative `/sitemaps/en` source URL target: `89`
- Latest external sitemap discovery count: `85`
- Latest IndexNow representative submission count: `88`
- Current representative feed source target item count: `62`
- Latest external feed publish item count: `62`
- The latest Search Console discovered count now reflects the latest externally submitted `85`-URL sitemap after the signed-in `bobob935@gmail.com` resubmission on `2026. 7. 7.`. The current source target is now `89` URLs after adding Open Graph social crawler review plus Base64 payload review and YAML/ENV deployment config review to the representative operations tool set, while the latest deployed IndexNow evidence remains the previous `88`-URL submission until the 89-URL target is deployed and submitted. A same-day `sc-domain:bobob.app` recheck also refreshed from the older visible `84`-page row to the current `85`-page row after reload. Later Chrome retries exposed only `task10@ljfriends.net` or a `bobob935@gmail.com` passkey authentication stop, so the current matrix still uses the successful `bobob935@gmail.com` observation until a valid 89-URL Search Console row is observed. This is discovery evidence, not indexing proof.
