...
- Current submitted sitemap URL count: `138`
- Current feed item count: `127`
- Current Blog count: `103`
...
| RSS feed | `/feed.xml` | Live feed has `127` Blog + Play items and WebSub hub discovery | Feed readers and lightweight crawlers can discover current content | It does not replace sitemap or webmaster-tool evidence |
| Atom feed | `/atom.xml` | Live feed has `127` Blog + Play entries and WebSub hub discovery | Feed readers and WebSub can discover current content | It does not prove search indexing |
| JSON Feed | `/feed.json` | Live feed has `127` Blog + Play items and WebSub hub metadata | Programmatic readers can discover current content | It does not broaden the submitted sitemap scope |
| WebSub | `https://pubsubhubbub.appspot.com/` for RSS and Atom topics | Latest publish responses `204`, `204` with feed item counts `127`, `127` after the brick-breaker landing guide update | Feed update topics were accepted by the hub | It does not prove Google indexed the URLs |
...
- Current live `/sitemaps/en` URL count: `138`
- Latest IndexNow URL count: `138`
- Latest feed item count: `127`
