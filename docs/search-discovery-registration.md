...
| --- | --- | --- | --- | --- |
| Google Search Console sitemap | `/sitemaps/en` under the `https://www.bobob.app/` property | Current production sitemap target: `75` URLs after the office-survival representative promotion once this source target is deployed; latest deployed/submitted evidence still reflects the previous 74-URL set because the latest Vercel deployment is rate limited. Latest signed-in Search Console observation after resubmission of the previous 77-URL deployment: `/sitemaps/en`, submitted `2026. 7. 3.`, last read `2026. 7. 3.`, status `성공`, discovered pages `77`, videos `0` | Google accepted, read, and discovered the previous 77-URL canonical sitemap set | It does not prove indexing, impressions, ranking, that the representative Blog + Play pages are search-ready, or that the new 75-URL production target has been resubmitted and observed in Search Console |
...
| WebSub | `https://pubsubhubbub.appspot.com/` for RSS and Atom topics | Latest WebSub representative publish: `59` RSS items and `59` Atom entries with response statuses `204`, `204` after the bug-clicker archive consolidation refresh | Feed update topics were refreshed for the current representative feed set | It does not prove Google indexed the URLs |
...
- Current representative `/sitemaps/en` source URL target: `75`
- Latest external sitemap discovery count: `77`
- Latest IndexNow representative submission count: `74`
- Current representative feed source target item count: `60`
- Latest external feed publish item count: `59`
- The latest Search Console discovered count still reflects the previous `77`-URL sitemap observation. The current source target is now `75` sitemap URLs and `60` feed items after the office-survival promotion, while the latest deployed/submitted external evidence still reflects the previous `74`-URL / `59`-item set until the deployment rate limit clears and `/sitemaps/en` is resubmitted and observed there.
...
- Indexed pages: `0`
- Not indexed pages: `5`
- `/sitemaps/en` discovered pages: `77` against the previously submitted `77`-URL sitemap; this proves discovery alignment for that earlier deployment, not indexing or the current 75-URL production target.
