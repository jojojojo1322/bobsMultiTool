...
- Latest external feed publish item count: `59`
- The latest Search Console discovered count still reflects the previous `77`-URL sitemap observation. The current source target is now `75` sitemap URLs and `60` feed items after the lucky-scratch duplicate representative consolidation, while the latest deployed/submitted external evidence still reflects the previous `74`-URL / `59`-item set until the new source target is deployed and `/sitemaps/en` is resubmitted and observed there.
