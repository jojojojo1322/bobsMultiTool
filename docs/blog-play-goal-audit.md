...
- Category distribution: the original five categories each have at least `7` posts, and the `정보` lane has `21` date-sensitive posts.
- Date-sensitive information lane: `21` posts live under `정보` for AI plan prices, safe-driving insurance discounts, MacBook price/outlook notes, football checks, World Cup bracket/time checks, and youth savings notes.
- Standalone Blog lane: `31` posts have no forced `relatedPlay`, so Blog is not subordinate to Play.
...
- IndexNow:
  - Latest submitted URL count: `106`
  - Latest response status: `200`
  - The 106-URL live sitemap set was submitted after the Play canvas upgrade, password guess preview, follow-up youth savings result expansion, sum-box combo bonus, and the latest football/youth-savings information notes.
- Bing:
...
- Search Console sitemap discovery must be checked again against the new live sitemap URL count (`106`), and indexing is still unresolved.
