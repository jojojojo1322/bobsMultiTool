...
- Category distribution: the original five categories each have at least `7` posts, and the `정보` lane has `17` date-sensitive posts.
- Date-sensitive information lane: `17` posts live under `정보` for AI plan prices, safe-driving insurance discounts, MacBook price/outlook notes, football checks, World Cup bracket checks, and youth savings notes.
- Standalone Blog lane: `27` posts have no forced `relatedPlay`, so Blog is not subordinate to Play.
...
- IndexNow:
  - Latest submitted URL count: `99`
  - Latest response status: `200`
  - The 99-URL live sitemap set was submitted after the Play canvas upgrade, memory replay fix, and follow-up bracket information expansion.
- Bing:
...
- Search Console sitemap discovery must be checked again against the new live sitemap URL count (`99`), and indexing is still unresolved.
