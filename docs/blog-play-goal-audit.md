...
- Search Console account/property guard source target:
  - The existing search-discovery and not-indexed URL workflows now route `search console wrong google account`, `search console property access denied`, `search console account mismatch`, `Search Console 계정 다름`, and `이 속성에 액세스할 수 없습니다` into the current HTTP/Sitemap/Robots/Meta/URL Parser path.
  - `/tools/sitemap-generator` now carries the same Search Console account/property guard query family in search intents, aliases, and keywords so an access-denied state lands on the sitemap/search-discovery report rather than a thin account-only page.
  ...
  - This source change records the `2026-07-08` external retry state but is not Search Console, Bing, Naver, traffic, or indexing proof until later signed-in external observations change.
