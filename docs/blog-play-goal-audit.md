...
- Category distribution: the original five categories each have at least `7` posts, and the `정보` lane has `32` date-sensitive posts.
- Date-sensitive information lane: `32` posts live under `정보` for AI plan prices, safe-driving insurance discounts, MacBook price/outlook notes, football checks, World Cup bracket/time/search/watch/round-of-32/ticket/host-city checks, and youth savings notes.
- Standalone Blog lane: `42` posts have no forced `relatedPlay`, so Blog is not subordinate to Play.
...
- WebSub:
  ...
  - Latest feed item counts: `112`, `112`
...
- Search Console sitemap discovery must be checked again against the new live sitemap URL count (`123`), and indexing is still unresolved.
