...
- Tools position: selected high-utility tools for URL status, redirect chains, response headers, DNS, sitemap, robots, meta/canonical, JWT auth handoff reports, and JSON/API inspection are now presented as the main homepage workflow. Generic developer utilities remain available under `/tools` but should not read as a thin DevTools clone.
