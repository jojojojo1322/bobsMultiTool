...
- Category distribution: the original five categories each have at least `7` posts, and the `정보` lane has `18` date-sensitive posts.
- Date-sensitive information lane: `18` posts live under `정보` for AI plan prices, safe-driving insurance discounts, MacBook price/outlook notes, football checks, World Cup bracket/time checks, and youth savings notes.
- Standalone Blog lane: `28` posts have no forced `relatedPlay`, so Blog is not subordinate to Play.
...
- IndexNow:
  - Latest submitted URL count: `101`
  - Latest response status: `200`
  - The 101-URL live sitemap set was submitted after the Play canvas upgrade, sum-box hint highlight, and follow-up World Cup time expansion.
- Bing:
...
- Search Console sitemap discovery must be checked again against the new live sitemap URL count (`101`), and indexing is still unresolved.
