...
- ENV deployment report source target:
  ...
  - This source change is implemented in commit `45e74775`, with secret-like key-name redaction tightened in commit `1120499d`. The final production deployment refreshed live discovery, search-discovery registration, indexing-observation, submitted URL health, IndexNow, and production browser verification at the same `85` sitemap URLs and `62` feed items. It is deploy-config handoff evidence only and does not prove Google, Bing, or Naver indexed the checked URLs.
...
- Search Console sitemap discovery still matches the previous externally submitted representative sitemap URL count (`85`), but the current live/source representative sitemap URL count is now `86` after adding `/tools/yaml-validator`.
- The current source sitemap target is now `86` after adding YAML Validator to the representative operations tool set, so the next Search Console/Bing/Naver pass must compare whether external discovery catches up and whether indexing/report changes appear instead of treating sitemap discovery as indexing proof.
