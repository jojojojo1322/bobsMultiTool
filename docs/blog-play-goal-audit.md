...
- Global search evidence surface: `/search?q=` now returns `data-content-search-workflows` results for task-shaped operations queries such as redirect debugging, search discovery readiness, Search Console report-refresh delay checks, Naver sitemap-not-found checks, DNS deployment, and security-header review before falling through to individual tool cards, so internal search no longer reads as only a generic developer-tool archive.
...
- Page indexing report freshness source target:
  ...
  - This source change has passed local search, Blog/Play, goal-audit, indexing-followup, registry, localization/i18n, content, lint, build, route, submitted URL health, rendered-content, and Playwright checks, but it is not Search Console indexing proof until a later signed-in observation changes.
