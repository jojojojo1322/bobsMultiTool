...
- Representative submitted count: `31` Blog posts.
- Archive/noindex candidate count: `89` Blog posts.
...
- Representative information lane: `14` posts remain submitted under `정보`.
- Standalone Blog lane: `47` posts have no forced `relatedPlay`, so Blog is not subordinate to Play.
- Representative standalone lane: `14` submitted posts have no forced `relatedPlay`.
...
- Date-sensitive information pruning: `world-cup-search-terms-check-2026-06-27`, `world-cup-player-stats-check-2026-06-27`, `world-cup-round-of-32-korea-time-2026-06-27`, and `youth-future-savings-search-questions-2026-06-27` are now archive/noindex candidates so the submitted set is less dominated by duplicate information wrappers.
...
- Google Search Console:
  ...
  - The representative sitemap was intentionally reduced and Search Console discovered pages matched the previous `76` URL count. This is discovery evidence, not indexing proof, and it does not yet cover the new `72` URL source target.
...
- IndexNow:
  ...
  - The previous 76-URL representative sitemap set has been submitted to IndexNow with response status `200`; the new 72-URL source target has not been submitted yet.
- Bing:
...
- WebSub:
  ...
  - Latest published feed item counts: `61`, `61`; the new source target is `57`.
...
- Search Console sitemap discovery matched the previous reduced representative sitemap URL count (`76`), but this only proves Google read the prior submitted sitemap; the new source target is `72` and still needs deployment/resubmission.
